...
class GameUtils {
    ...
    */
    ...
    static func keepCircleCenterInRange(center: CGFloat, range: ClosedRange<CGFloat>, radius: CGFloat) -> CGFloat {
        var centerInRange : CGFloat = 0.0
        if (center - radius <= range.lowerBound ) {
            centerInRange = range.lowerBound + radius
        } else if (center + radius >= range.upperBound ) {
            centerInRange = range.upperBound - radius
        } else {
            centerInRange = center
        }
